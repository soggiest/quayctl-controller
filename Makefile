...
TAG?=latest
PREFIX?=quay.io/nicholas_lane/quayctl-controller
TEMP_DIR:=$(shell mktemp -d)

quayctl-controller: main.go 
	CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -ldflags '-w' -o quayctl-controller main.go	
